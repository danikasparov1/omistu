...
setup() {
    ...
    async edit_delete_chart_item(event){
         ...
         this.dialog.add(ConfirmationDialog, {
            ...
            confirm: () => {
               self.rpc('/custom_dashboard/dashboard_item_delete', {
                  chart_id: chart_id,
                })
                    .then(function(result){

                         self.rpc('/custom_dashboard/update_chart_item_dashboard', {
            ...
            var options = {
                on_close: function () {
                    self.rpc("/custom_dashboard/update_chart_item_dashboard", {dashboard_id: self.dashboard_id})
                    .then(function(res){
                        self.actionService.doAction(res);
                    });
                },
...
//                ajax.jsonRpc("/custom_dashboard/get_dashboard_items_data", 'call', {'dashboard_item_id': dashboard_item_id})
                self.rpc('/custom_dashboard/get_dashboard_items_data', {
...
//                    boundaryGap: false,
    ...
    async generate_kpi_or_tiles(dict_data, dashboard_item_id){
        ...
        async save_dashboard_data(event){
            ...
            this.rpc("/custom_dashboard/dashboard_configuration", {data_list: JSON.stringify(data_list)})
            var dashboard_template_name = $('#dashboard_template_name_input').val();
            this.rpc("/custom_dashboard/dashboard_template_name",{dashboard_id: self.dashboard_id, template_name: dashboard_template_name})
            .then(function(result){
        ...
        async add_chart_item(event){
            ...
            var options = {
                on_close: function () {
                    this.rpc("/custom_dashboard/update_chart_item_dashboard",{dashboard_id: self.dashboard_board_id})
                    .then(function(res){
                        self.actionService.doAction(res);
                    });
                },
        ...
        async _render_chart(){
            ...
            self.dashboard_data.dashboard_item_ids.forEach(function (dashboard_item_id) {
                self.rpc("/custom_dashboard/get_dashboard_items_data",{dashboard_item_id: dashboard_item_id})
                .then(function (res) {
                    if (['kpi','tiles'].includes(res.chart_type)){
                        self.generate_kpi_or_tiles(res,dashboard_item_id)
                    }
...
//         this.do_action('eg_ai_smart_dashboard_lite.action_custom_dashboard_board');
        ...
        async clear_search(){
            ...
            this.rpc("/custom_dashboard/remove_search_chart",{dashboard_item_id: self.dashboard_id})
            .then(function(res){
               $(".grid-stack").remove();
        ...
        async change_chart_search(){
            ...
            this.rpc("/custom_dashboard/search_input_chart",{search_input: search_input, dashboard_item_id: self.dashboard_id})
            .then(function(res){
                $(".grid-stack").remove();
                $(".container-fluid-dashboard").append("<div class='grid-stack' id='grid-stack'></div>")
                self.dashboard_data = res;
