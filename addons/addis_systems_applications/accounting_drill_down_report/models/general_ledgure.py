...
class ReportGeneralLedger(models.AbstractModel):
    _name = 'addissystems.report_generaledgure'
    ...
    def get_lines(self,form_data):
        ...
        if lines.ids:
            ...
            for acct in accounts:
                ...
                lines_under_acct=self.env['account.move.line'].search(domain+[('account_id','=' ,acct[0])],order="date")
                for line in lines_under_acct:
                    line_values={
                        "id":line.id,
                    ...
                    data[acct[0]]['data_lines'].append(line_values)
        return data
...
class GeneralLedgureExcelReport(models.AbstractModel):
    _name="report.accounting_drill_down_report.report_generalledgure_xlsx"
    _inherit = 'report.report_xlsx.abstract'
    def generate_xlsx_report(self, workbook, data, partners):
        report_name="General Ledgure"
        ...
        index=2
        headers = ["Account","Date", "Partner","Debit", "Credit", "Balance"]
        for col, header in enumerate(headers):
            sheet.write(index, col, header, header_format)
        for i,line in enumerate(data["data"]["lines"]):
             index+=1
             sheet.write(index,0,data["data"]["lines"][line]["label"],main_topic_format)
             sheet.write(index,3,data["data"]["lines"][line]["debit"],number_format)
             sheet.write(index,4,data["data"]["lines"][line]["credit"],number_format)
             sheet.write(index,5,data["data"]["lines"][line]["balance"],number_format)

             for j,line_2 in enumerate(data["data"]['lines'][line]['data_lines']):
                 index+=1
                 sheet.write(index,0,line_2['name'])
                 sheet.write(index,1,line_2['date'])
                 sheet.write(index,2,line_2['partner'])
                 sheet.write(index,3,line_2['debit'],number_format)
                 sheet.write(index,4,line_2['credit'],number_format)
                 sheet.write(index,5,line_2['balance'],number_format)
