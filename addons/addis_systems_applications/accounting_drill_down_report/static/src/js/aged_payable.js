...
export class AgedPayableReport extends BaseDrilldownReport {
    async setup() {
    ...
    async get_default_values(){
        ...
        this.state.date_from = DateTime.now()
    }
    async getxlsx(ev) {
        return this.action.doAction({
            'type': 'ir.actions.report',
            'report_type': 'xlsx',
            'report_name': 'accounting_drill_down_report.report_aged_payable_xlsx',
            'report_file': 'accounting_drill_down_report.report_aged_payable_xlsx',
            'data': {
                'report_name': "Aged Ledgure",
                'data': {
                    ...this.state,
    ...
    async print_pdf(ev) {
        return this.action.doAction({
            ...
            'report_name': 'accounting_drill_down_report.report_aged_payable',
            'report_file': 'accounting_drill_down_report.report_aged_payable',
            'data': {
    ...
    async get_financial_data(){
        this.state.lines = await this.orm.call("addissystems.report_agedpayable", "get_lines", [this.state]); 
        this.report_name = "Aged Partner Report"
        console.log("check final",this.state.lines)
...
AgedPayableReport.template = "accounting_drill_down_report.aged_payable_report_owl";
actionRegistry.add("addisystems_aged_payable", AgedPayableReport);