...
export class CashBookReport extends BaseDrilldownReport {
    async setup() {
        super.setup();
        ...
        const accounts = await this.orm.call("addissystems.report_cashbook", "get_default_account_ids", []); 
    ...
    async getxlsx(ev) {
        return this.action.doAction({
            ...
            'report_type': 'xlsx',
            'report_name': 'accounting_drill_down_report.report_cashbook_xlsx',
            'report_file': 'accounting_drill_down_report.report_cashbook_xlsx',
            'data': {
                'report_name': "Cash Book xlsx",
                'data': {
                    ...this.state,
    ...
    async print_pdf(ev) {
        return this.action.doAction({
            ...
            'report_name': 'accounting_drill_down_report.report_cashbook',
            'report_file': 'accounting_drill_down_report.report_cashbook',
            'data': {
    ...
    async get_financial_data() {
        this.report_name = "Cash Book Report";
        this.state.lines = await this.orm.call("addissystems.report_cashbook", "get_lines", [this.state]);
    }
}

CashBookReport.template = "accounting_drill_down_report.cash_book_report_owl";
actionRegistry.add("addisystems_cash_book", CashBookReport);