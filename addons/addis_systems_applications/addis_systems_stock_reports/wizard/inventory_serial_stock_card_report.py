...
class AddisSystemsSerialStockCardReport(models.TransientModel):
    _name = 'report.stock.move.serial.stock.card'
    _description = 'Addis Systems Serial Stock Card'
    ...
    lot_serial = fields.Many2many(comodel_name='stock.lot',string="Lot/Serial Number")
    ...
    def _stock_moves(self):
        domain = [('state', '=', 'done'),('lot_id','!=',False)]
        ...
        if self.lot_serial:
            domain+=[('lot_id','in',self.lot_serial.ids)]
    ...
    def preview_html(self):
        if self.product_id and self.product_category:
            self.product_category = None
        return self.env.ref('addis_systems_stock_reports.report_addis_systems_serial_stock_card_report_html').report_action(self)

    def process_pdf(self):
        if self.product_id and self.product_category:
            self.product_category = None
        return self.env.ref('addis_systems_stock_reports.report_addis_systems_serial_stock_card_report_pdf').report_action(self)
    ...
    def process_excel(self):
        ...
        return {
            ...
            'report_type': 'excel',
            'data': {
                'model': 'report.stock.move.serial.stock.card',
                'options': json.dumps(stock_card_data, default=date_utils.json_default),
                'output_format': 'xlsx',
                'report_name': f"Serial Stock Card - {self.stock_location.complete_name} {self.product_category.name or self.product_id.name}"
            }
        }
    ...
    def get_xlsx_report(self, data):
        ...
        title = f" Serial Stock Card - {stock_location.complete_name} From {date_from} - {date_to}" if date_from and date_to else f"Stock Card - {stock_location.complete_name} All"
        ...
        sheet.merge_range('C3:C4', "Lot/Serial Number", head)
        sheet.merge_range('D3:F3', "Quantity", head_center)
        sheet.write(3, 3, "Received", head_center)
        sheet.write(3, 4, "Issued", head_center)
        sheet.write(3, 5, "Balance", head_center)
        sheet.merge_range('G3:G4', "Partner", head_center)
        ...
        if product_category and category_products:
            for product in category_products:
                ...
                if product_move_lines != 0:
                    ...
                    sheet.merge_range(f'A{row_index}:G{row_index}', f"[{product.default_code}] {product.name}" if product.default_code else str(product.name), cell_format_left_bold)
                    for line in self.category_product_moves(product, product_moves):
                        ...
                        sheet.write(row_index, 2, line.lot_id.name, cell_format_left)
                        sheet.write(row_index, 3, received if received != 0 else '', cell_format_right)
                        sheet.write(row_index, 4, issued if issued != 0 else '', cell_format_right)
                        sheet.write(row_index, 5, quantity_on_hand, cell_format_right)
                        sheet.write(row_index, 6, line.picking_id.partner_id.name if line.picking_id.partner_id.name not in [0, False, ''] else '', cell_format_left)
                        row_index = row_index + 1
