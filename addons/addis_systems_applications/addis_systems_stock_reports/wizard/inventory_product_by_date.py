...
class InventoryCategorizedDate(models.TransientModel):
    _name="inventory.product.date.wizard"
    ...
    def get_products_under_date(self,categ_name):
        current_date=categ_name
        next_date=categ_name+ + timedelta(days=1)
        domain = self.get_constant_domain()
        domain+=[("date",">=",current_date),("date","<",next_date)]
    ...
    def preview_html(self):
        ...
        return self.env.ref('addis_systems_stock_reports.action_print_report_inventory_product_date_html').report_action(self,data=data)
    ...
    def process_pdf(self):
        ...
        data['form']=self.read(["date_from","date_to","warehouse_ids","stock_location_ids","product_ids","product_categ_ids","state"])[0]
        return self.env.ref('addis_systems_stock_reports.action_print_report_inventory_product_date_pdf').report_action(self,data=data)
    ...
    def process_excel(self):
        ...
        return {
            ...
            'data': {
                ...
                'output_format': 'xlsx',
                'report_name': f"Inventory Summary By Date"
    ...
    def get_xlsx_report(self, data):
        ...
        report_name = "Stock Summary by Date"
        ...
        # Header Row
        index += 1
        headers = ["Date Category", "Product", "Reference", "Quantity"]
        for col, header in enumerate(headers):
            sheet.write(index, col, header, header_format)
        
        # Data Rows
        index += 1
        for categ in wizard.get_product_dates():
            sheet.write(index, 0, categ[0].strftime("%B %d, %Y"), cell_format)
            sheet.write(index, 3, categ[1], number_format)
            index+=1
            for product in wizard.get_products_under_date(categ[0]):
                sheet.write(index, 1, product[0].name, cell_format)
                sheet.write(index, 3, product[1], number_format)
                index += 1
                for reference in wizard.get_references(product[0].id,categ[0]):
