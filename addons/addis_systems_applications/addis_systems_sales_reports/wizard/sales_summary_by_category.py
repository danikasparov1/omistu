...
class SalesSummaryReportByCategory(models.TransientModel):
    _name = 'sales.summary.report.category'
    ...
    def return_data_saleorder(self,move_lines):
        mydata={
    ...
    def return_data_invoice(self,move_lines):
        mydata={
    ...
    def check_quotation(self):
        ...
        order_lines=self.env["sale.order.line"].search([('order_id',"in",sales.ids)])
        data=self.return_data_saleorder(order_lines)
        return {"result":data}
    
    def check_saleorder(self):
        domain=[("state","=","sale")]
        if self.date_from:
            domain += [('date_order', '>=', self.date_from)]
        if self.date_to:
    ...
    def check_invoice(self):
        ...
        data=self.return_data_invoice(move_lines)
        return {"result":data}        

    def check(self):
        if self.type=="saleorder":
            return self.check_saleorder()
        elif self.type =="invoice":
            return self.check_invoice()
        elif self.type =="qotation":
            return self.check_quotation()
        domain=[]
        if self.date_from:
            domain += [('date', '>=', self.date_from)]
        if self.date_to:
    ...
    def generate_report_category(self):
        return self.env.ref('addis_systems_sales_reports.sales_sales_summary_report_report_category').report_action(self)
