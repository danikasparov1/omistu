...
class SalesSummaryReportByCustomer(models.TransientModel):
    _name = 'sales.summary.product.summary'
    ...
    def get_product_content_sales(self,invoice_date,product):
        ...
        move_lines=self.env["sale.order.line"].search([('order_id','in',acct_moves.ids),('product_template_id','=',product.id)])

        return move_lines

    def get_product_content(self,invoice_date,product):
        if self.type != "invoice":
            return self.get_product_content_sales(invoice_date,product)
        ...
        move_lines=self.env["account.move.line"].search([('display_type','=','product'),('move_id','in',acct_moves.ids),('product_id','=',product.id)])
        return move_lines
    ...
    def get_moves_sales(self,invoice_date):
        ...
        if self.user_id:
            domain+=[('create_uid', '=', self.user_id.id)]
        acct_moves=self.env["sale.order"].search(domain)
        products=self.env["sale.order.line"].search([('order_id','in',acct_moves.ids)]).product_template_id
        return products
    ...
    def get_moves(self,invoice_date):
        ...
        if self.user_id:
            domain+=[('create_uid', '=', self.user_id.id)]
        acct_moves=self.env["account.move"].search(domain)
        products=self.env["account.move.line"].search([('display_type','=','product'),('move_id','in',acct_moves.ids)]).product_id
        return products
    ...
    def generate_report(self):
        return self.env.ref('addis_systems_sales_reports.addis_sales_summary_product_summary').report_action(self)